...
names = ['t/s', 'ΔT/°C']
df = pd.read_excel('Sucrose.xlsx', skiprows=0, names=names)
print(df)
...
Time = FindTime(lambda x: lin(x, a1, b1), lambda x: lin(x, a2, b2), curve, df['t/s'][18], df['t/s'][44], df['t/s'][16], df['t/s'][46])


# 准备作图
plt.figure(figsize=(6, 4.5), dpi=600)
plt.axis([0, 800, -0.1, 1.2])


# 数据-时间曲线
plt.plot(startx, lin(startx, a1, b1), linewidth=1, color=[0.7,0.2,0.2])
plt.plot(endx, lin(endx, a2, b2), linewidth=1, color=[0.7,0.2,0.2])

left, right = df['t/s'][16], df['t/s'][46]
...
plt.plot(np.linspace(250, 720, 10), lin(np.linspace(250, 720, 10), a2, b2), linewidth=1, color=[0.7,0.2,0.2], linestyle='--')
plt.vlines(Time, ymin=-0.1, ymax=lin(Time, a2, b2), linewidth=1, color=[0.7,0.2,0.2], linestyle='--')
plt.vlines(df['t/s'][16], ymin=-0.1, ymax=df['ΔT/°C'][16], linewidth=1, color=[0.7,0.2,0.2], linestyle='--')
plt.vlines(df['t/s'][46], ymin=-0.1, ymax=df['ΔT/°C'][46], linewidth=1, color=[0.7,0.2,0.2], linestyle='--')
...
plt.yticks(np.linspace(0.0, 1.20, 9))
plt.grid(True, linewidth=0.5)
plt.savefig("Sucrose.png", transparent=True)